...
func startPeer(tr http.RoundTripper, urls types.URLs, local, to, cid types.ID, r Raft, fs *stats.FollowerStats, errorc chan error, v3store storage.KV, term uint64) *peer {
	picker := newURLPicker(urls)
	status := newPeerStatus(to)
	p := &peer{
		id:            to,
		r:             r,
		msgAppWriter:  startStreamWriter(to, status, fs, r, v3store),
		writer:        startStreamWriter(to, status, fs, r, v3store),
		msgSnapWriter: startStreamWriter(to, status, fs, r, v3store),
	...
	msgSnapReader := startStreamReader(tr, picker, streamTypeMsgSnap, local, to, cid, status, p.recvc, p.propc, errorc, term)
	go func() {
		var paused bool
		for {
			select {
			case m := <-p.sendc:
				...
				if isMsgSnap(m) {
					writec, ok := p.msgSnapWriter.writec()
					if !ok {
